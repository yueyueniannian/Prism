...
function showInsights() {
  elements.insightStack.classList.remove("is-hidden");
}

function hideInsights() {
  elements.insightStack.classList.add("is-hidden");
}
...
function initPageTransitions() {
  ...
  transitionLinks.forEach((link) => {
    link.addEventListener("click", (event) => {
      ...
      event.preventDefault();
      showTransitionSplash(link.dataset.transition || "");
      window.setTimeout(() => {
...
function showTransitionSplash(mode) {
  const existing = document.getElementById("transitionSplash");
  if (existing) {
    existing.remove();
  }

  const splash = document.createElement("div");
  splash.id = "transitionSplash";
  splash.className = "transition-splash";

  const mark = mode === "research-map" ? "RM" : "PA";
  const title = mode === "research-map" ? "Research Map Lab" : "Paper Audit Lab";
  const sub = mode === "research-map"
    ? "正在进入研究方向图谱"
    : "正在进入论文评估工作台";
